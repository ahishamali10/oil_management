...
login.call = function(args, callback) {
	login.set_indicator('التحقق...', 'blue');
	...
	var login_handlers = {
		200: function(data) {
			if(data.message == 'Logged In'){
				login.set_indicator('تم بنجاح', 'green');
				...
				if(data.message==='not found') {
					login.set_indicator('المستخدم غير موجود', 'red');
				} else if (data.message=='not allowed') {
					login.set_indicator('غير مسموح', 'red');
				} else if (data.message=='disabled') {
					login.set_indicator('غير مسموح: المستخدم غير فعال', 'red');
				} else {
					login.set_indicator('التعليمات ارسلت بالايميل', 'green');
				}
		...
		401: get_error_handler('تسجيل الدخول غير صالح. حاول مرة أخرى.'),
		417: get_error_handler('للأسف!! حدث خطاء')
	};
